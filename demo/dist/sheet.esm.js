...
//#region node_modules/@magic-spells/dialog-panel/dist/dialog-panel.esm.js
/**
...
*/
var DialogPanel = class DialogPanel extends HTMLElement {
	#state = "hidden";
	#triggerElement = null;
	#dialog = null;
	#result = null;
	...
	connectedCallback() {
		...
		if (!_.#dialog) {
			console.warn("DialogPanel: No <dialog> element found inside <dialog-panel>");
			return;
		}
	...
	disconnectedCallback() {
		...
		_.#unwireMorphEngine();
		if (_.#handlers.click) _.removeEventListener("click", _.#handlers.click);
		if (_.#dialog) {
			if (_.#handlers.dialogClick) _.#dialog.removeEventListener("click", _.#handlers.dialogClick);
			if (_.#handlers.cancel) _.#dialog.removeEventListener("cancel", _.#handlers.cancel);
			if (_.#handlers.close) _.#dialog.removeEventListener("close", _.#handlers.close);
		}
	}
	/**
	...
	*/
	#bindEvents() {
		...
		_.#handlers.close = () => {
			if ((_.#state === "showing" || _.#state === "shown") && _.#morphEngine?.animatesDialog && !_.#dialog.open) {
				_.#result = _.#dialog.returnValue || _.#result;
				_.#morphEngine.stop();
				if (_.#state !== "hidden") _.#finalizeHidden();
				return;
			}
			const forceClosedShown = _.#state === "shown" && !_.#dialog.open;
			const forceClosedShowing = _.#state === "showing" && _.#pendingRAF !== null && !_.#dialog.open;
	...
	*/
	#handleMorphReveal(detail) {
		const _ = this;
		if (_.#state === "showing" && detail?.to === _.#dialog && !_.#dialog.open) _.#dialog.showModal();
	}
	/**
	...
	*/
	show(triggerEl = null) {
		...
		const reversingMorph = _.#state === "hiding" && engine?.state === "hiding";
		...
		if (engine && (direct || triggerEl || reversingMorph)) {
			...
			if (direct && !_.#dialog.open) _.#dialog.showModal();
			if (direct && engine.state === "shown" && _.#state === "showing") _.#handleMorphShown();
			return true;
		}
		_.#dialog.showModal();
	...
	*/
	hide(triggerEl = null) {
		...
		const reversingMorph = _.#state === "showing" && engine?.state === "showing";
		if (_.#state === "showing" && !reversingMorph && !direct) return false;
		_.#result = triggerEl?.dataset?.result ?? null;
		if (!_.#emit("beforeHide", {
			cancelable: true,
			result: _.#result,
...
* @extends HTMLElement
*/
var DialogBackdrop = class extends HTMLElement {
	#panel = null;
	#handlers = { click: null };
	connectedCallback() {
		const _ = this;
		...
		if (!_.#panel) {
			console.warn("DialogBackdrop: Must be inside a <dialog-panel> element");
			return;
		}
		_.#handlers.click = () => {
			_.#panel.hide();
...
if (!customElements.get("dialog-backdrop")) customElements.define("dialog-backdrop", DialogBackdrop);
if (!customElements.get("dialog-panel")) customElements.define("dialog-panel", DialogPanel);
//#endregion
