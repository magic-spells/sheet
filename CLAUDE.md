...
- **backdrop** — the sheet's tap branch plus a capture-phase `click` on dialog-panel that swallows outside clicks before dialog-panel's bubble handler. The guard gates on **target first, then geometry**. `event.target !== dialog` returns at once: a descendant can paint outside the dialog's box (a `position: fixed` child, a nested full-screen lightbox's close button or scrim), and because the guard is capture-phase, stopping its click there killed it before it reached its own control. That exemption is safe only because dialog-panel ≥ 2.0.2 applies the same target gate in its `dialogClick` — before it, a pointer-capture drag that began on content and released on the scrim arrived retargeted to the captor and dialog-panel hid the sheet on coordinates alone, so the guard had to catch it by geometry. What remains is judged by rect: the native `::backdrop` has no node, so a genuine scrim tap targets the *dialog* itself — as does a click on its own padding, and the retargeting-free release of a content press (a mouse text selection lands its click on the common ancestor, the dialog). Position separates the first two; `#scrimPress` separates the third.
...
Scrim gestures are therefore read off the dialog by geometry. `#outsideGuard` (capture-phase on `dialog-panel`, ahead of its bubble-phase `dialogClick`) refuses an outside click when the `dismiss` policy forbids `backdrop` **or** the gesture began inside the panel. `#scrimPress` records where `pointerdown` landed — a `click` is retargeted to whatever held pointer capture, so selecting text in the sheet and releasing past its edge used to close it. Starting position answers both cases without a distance gate, which would refuse the near-miss swipe; the old target-based exemption (`dialogRef.contains(target)`) is exactly what waved the captured release through — do not restore it.

**A press that never becomes a click must still expire.** `#outsideGuard`'s `finally` is the only consumer of a press and it needs a click to run, but a `pointerdown` does not guarantee one: native scrolling in the content cancels the stream, and a claimed drag can end with the click suppressed by movement. A stranded `'inside'` refuses the *next* scrim tap — which on iOS carries no `pointerdown` to re-arm it — so the bug returns on the first tap after any scroll. `pointercancel` clears immediately (no click can follow a cancelled stream); `pointerup` clears on a **macrotask**, never synchronously, because the retargeted click of an inside-press/outside-release arrives *after* pointerup and is exactly what the guard exists to refuse. A new `pointerdown` cancels a pending expiry, or it would fire mid-gesture and hand the guard a `'none'` it never saw.

**`#scrimPress` is three states, and `'none'` is not `'inside'`.** iOS Safari delivers a scrim tap as a bare `click` — target `<dialog>`, `detail: 1`, `pointerType: 'mouse'` — with **no preceding `pointerdown` at all**. As a boolean that tap was indistinguishable from a press inside the panel, so the guard refused it and backdrop dismissal was dead on iPhone and iPad, while swipe-to-dismiss (which never touches the scrim, only the panel's own `DragGesture`) kept working and masked it. Only `'inside'` refuses; `'none'` and `'scrim'` pass. The `finally` reset goes to `'none'` for the same reason — resetting to a refusing state would put the bug back on the second interaction.

Measuring this needs care, because **the observer is also the cure**: any document-level pointer *or* touch listener makes WebKit emit the full pointer sequence and the bug vanishes. A document `click` listener does not, a `touchstart` on the `<dialog>` does not, and `cursor: pointer` on the dialog does not. Instrument from `<dialog-panel>` or `<dialog>` — nodes the components already listen on — never from `document`. Playwright's WebKit with touch emulation does not reproduce it at all; synthetic taps bypass the path. This is a real-device check.
